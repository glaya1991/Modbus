...
uint16_t ParseErr = 0;
uint8_t Statistic[10]={0}; // buffer statistic
...
{   
    switch(modbus_sm){
        ...
        case PARSE_QUERY:                  
            ...
            UnRxTxSize = UnRxCnt;
            UnRxCnt = 0;                            // temp
            memset(&UnRxBuf, 0x00, UnRxTxSize);     // temp
            ...
            if(MODBUStrasact()){
                ...
                ParseErr++;
                Statistic[0] = ParseErr;
                Reg16[0] = Statistic[0];    // make function for copy statistic to modicon reg (Reg16)!!!
